...
COCO_MODEL_PATH = os.path.join(ROOT_DIR, "mask_rcnn_shapes_0020.h5")
...
class DSBDataset(utils.Dataset):
    ...
    def load_DSB(self, image_ids):
        ...
        for i, idx in enumerate(image_ids):
            self.add_image('shapes', idx, BASE_DIR + 'stage1_test/%s/images/%s.png' % (idx, idx))
    ...
    def load_mask(self, image_id):
        mask_ids = os.listdir(BASE_DIR + 'stage1_test/%s/masks' % self.image_info[image_id]['id'])
        shape = (self.image_size[0], self.image_size[1], len(mask_ids))
        masks = np.zeros(shape=shape)
        class_ids = []
        for i, idx in enumerate(mask_ids):
            mask = Image.open(BASE_DIR + 'stage1_test/%s/masks/%s' % (self.image_info[image_id]['id'], idx))
            # 之前注释了resize
            mask = mask.resize(self.image_size)
            mask = np.array(mask)
            mask[mask > 0] = 1
...
if __name__ == '__main__':
    ...
    dataset_val.load_DSB(IMAGE_IDS[63:])
    dataset_val.prepare()
