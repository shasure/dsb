...
def inference():
    if not os.path.exists('./result'):
        os.mkdir('./result')
    image_ids = os.listdir(BASE_DIR + 'stage2_test_final')
    images = []
    for idx in image_ids:
        image = Image.open(BASE_DIR + 'stage2_test_final/%s/images/%s.png' % (idx, idx))
    ...
    for image, idx in zip(images, image_ids):
        try:
            ...
            masks = r['masks']
            scores = r['scores']
            assert masks.shape[-1] == len(scores)
            # sort by scores
            score_order = np.argsort(scores)[::-1]
            occlusion = np.logical_not(masks[:, :, score_order[-1]]).astype(np.uint8)  # 没有被masks占据的像素为1
            for j in score_order[1:]:
                masks[:, :, j] = masks[:, :, j] * occlusion  # 尽量占据为0的像素
                occlusion = np.logical_and(occlusion, np.logical_not(masks[:, :, j]))  # 更新occlusion
                mask = masks[:, :, j]
            ...
            skip_image_ids.append(idx)
    print('skip_image_ids')
    print(skip_image_ids)
