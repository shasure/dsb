...
def non_max_suppression(boxes, scores, threshold):
    ...
    while len(ixs) > 0:
        ...
        # indicies into ixs.
        # 相当于是condition.nonzero(),[0]是因为iou只有一维，+1是因为iou不包括picked box
...
class Dataset(object):
    ...
    def prepare(self, class_map=None):
        ...
        self.class_ids = np.arange(self.num_classes)  # new_cls_id
        self.class_names = [clean_name(c["name"]) for c in self.class_info]  # 按照class添加的顺序对class自动编号
        self.num_images = len(self.image_info)
        self._image_ids = np.arange(self.num_images)  # 所有图片按照添加顺序编号

        # 用户设置的class id和新分配的new_cls_id的映射, {dataset.old_cls_id: new_cls_id}
        self.class_from_source_map = {"{}.{}".format(info['source'], info['id']): id
                                      for info, id in zip(self.class_info, self.class_ids)}

        # Map sources to class_ids they support
        self.sources = list(set([i['source'] for i in self.class_info]))
        self.source_class_ids = {}  # {source_dataset: new_cls_id}
        # Loop over datasets
